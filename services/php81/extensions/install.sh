...
if [[ -z "${EXTENSIONS##*,ssh2,*}" ]]; then
    ...
    if [[ "$?" = "1" ]]; then
        ...
        echo "ssh2 requires PHP >= 8.0.0, installed version is ${PHP81_VERSION}"
    fi
fi
...
if [[ -z "${EXTENSIONS##*,protobuf,*}" ]]; then
    ...
    if [[ "$?" = "1" ]]; then
        ...
        echo "yar requires PHP >= 8.0.0, installed version is ${PHP81_VERSION}"
    fi
fi
...
if [[ -z "${EXTENSIONS##*,pdo_sqlsrv,*}" ]]; then
    ...
    if [[ "$?" = "1" ]]; then
        ...
        echo "pdo_sqlsrv requires PHP >= 8.0.0, installed version is ${PHP81_VERSION}"
    fi
fi
