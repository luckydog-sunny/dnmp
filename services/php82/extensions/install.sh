...
echo "PHP version               : ${PHP82_VERSION}"
echo "Extra Extensions          : ${PHP82_EXTENSIONS}"
...
if [[ -z "${EXTENSIONS##*,ssh2,*}" ]]; then
    ...
    if [[ "$?" = "1" ]]; then
        ...
        echo "ssh2 requires PHP >= 8.0.0, installed version is ${PHP82_VERSION}"
    fi
fi
...
if [[ -z "${EXTENSIONS##*,protobuf,*}" ]]; then
    ...
    if [[ "$?" = "1" ]]; then
        ...
        echo "yar requires PHP >= 8.0.0, installed version is ${PHP82_VERSION}"
    fi
fi
...
if [[ -z "${EXTENSIONS##*,pdo_sqlsrv,*}" ]]; then
    ...
    if [[ "$?" = "1" ]]; then
        ...
        echo "pdo_sqlsrv requires PHP >= 8.0.0, installed version is ${PHP82_VERSION}"
    fi
fi
...
if [[ -z "${EXTENSIONS##*,swoole,*}" ]]; then
    ...
    isPhpVersionGreaterOrEqual 8 0
    if [[ "$?" = "1" ]]; then
        installExtensionFromTgz swoole-5.0.2 --enable-openssl --enable-http2 --enable-swoole-curl
    fi
fi
